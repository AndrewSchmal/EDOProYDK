...
def get_card_id(card_name, format_name=None):
    ...
    Includes format restriction and searches for the oldest legal version of the card for the format.
    """
    try:
        ...
        if response.status_code == 200:
            ...
            # Find the oldest version of the card (use 'GOAT' cutoff year as an example)
            if format_name and format_name.lower() == "goat":
                cutoff_year = 2005  # GOAT format cutoff year
                legal_versions = [
                    card for card in data
                    if 'banlist_info' in card and
                    card['banlist_info'].get('goat_format') == 'legal' and
                    int(card['card_sets'][0]['set_year']) <= cutoff_year
                ]
                if legal_versions:
                    # Use the oldest version available
                    return str(legal_versions[0]['id'])
            
            # Default: return the first version of the card if no filtering is applied
